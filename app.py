...
# Investor profile question keys (no default preselection)
_question_keys = [
...
st.sidebar.caption("Using default ESG dataset.")
...
profile_complete = all(st.session_state.get(k) is not None for k in _question_keys)
...
with tab0:
    st.caption(
        "Complete your investor profile below. "
        "Your answers shape the portfolio optimisation through your risk aversion score (gamma) and ESG preference score (lambda)."
    )
    with st.expander("About You", expanded=True):
        st.radio(
            "1. How experienced are you with investing?",
        ...
        st.radio(
            "3. When do you expect to need this money?",
        ...
        st.radio(
            ...
            ["A: None", "B: Less than 3 months of expenses", "C: 36 months", "D: More than 6 months"],
            key="q_safety",
            index=None,
        )
    with st.expander("Risk Preference", expanded=True):
        st.markdown("Choose the portfolio you prefer:")
        st.radio(
            "Round 1",
        ...
        st.radio(
            "If the stock market suddenly dropped by 30%, how would you react?",
            [
    ...
    with st.expander("ESG Preferences", expanded=True):
        likert_5 = [
            "1 - Strongly disagree",
            "2 - Disagree",
            "3 - Neutral",
        ...
        st.caption("Use a 1 to 5 scale: 1 = strongly disagree, 5 = strongly agree.")
        st.markdown("**Environmental**")
        st.radio(
            "I prefer companies with lower carbon emissions.",
            likert_5,
            key="q_env_1",
            index=None,
        )
        st.radio(
            "I am willing to accept slightly lower returns for stronger environmental performance.",
            likert_5,
            key="q_env_2",
            index=None,
        )
        st.markdown("**Social**")
        st.radio(
            "I care about labour standards and employee welfare.",
            likert_5,
            key="q_soc_1",
            index=None,
        )
        st.radio(
            "I prefer firms with strong diversity and human rights practices.",
            likert_5,
            key="q_soc_2",
            index=None,
        )
        ...
        st.radio(
            "Good board oversight and transparency are very important to me.",
            likert_5,
            key="q_gov_1",
            index=None,
        )
        st.radio(
            "I prefer firms with strong shareholder rights and ethical management.",
            likert_5,
            key="q_gov_2",
            index=None,
        )
        st.radio(
            "Which statement best reflects your attitudes toward ESG considerations relative to financial returns?",
            ["A: I focus only on financial returns, without considering ESG factors.", "B: ESG factors matter to me, but do not really influence my investment decisions.",
             "C: ESG considerations play an important role alongside financial returns.", "D: ESG impact is my top priority and I am willing to give up some return."],
            key="q_esg_imp",
            index=None,
        )
        st.radio(
            "To what extent does a higher ESG score make an investment more attractive to you?",
            ["A: Never - I would only consider ESG if the investment's financial performance is very strong.", "B: Slightly - I would include it if it does not negatively impact returns.",
             "C: Moderately - I value ESG and aim to increase ESG score by investing in companies with high ESG scores.", "D: Significantly - I always prioritise ESG and would choose investments with the highest ESG scores, even if returns are lower."],
            key="q_esg_att",
            index=None,
        )
    st.markdown("---")
    st.caption("Your current profile:")
    ...
    if profile_complete:
        ...
        render_metric_card(_pc3, "Risk aversion (gamma)", f"{gamma:.2f}")
        render_metric_card(_pc4, "ESG preference", f"{lambda_raw_avg:.2f} / 4")
    else:
        render_metric_card(_pc1, "Risk attitude", "Pending")
        render_metric_card(_pc2, "Sustainability profile", "Pending")
        render_metric_card(_pc3, "Risk aversion (gamma)", "Pending")
        render_metric_card(_pc4, "ESG preference", "Pending")
...
if st.session_state.get("run_optimisation", False):
    if not profile_complete:
        ...
        try:
            ...
            complete_portfolio = build_complete_portfolio(recommended, risk_free_rate, gamma)
            recommended_weights = get_weight_vector(recommended, tickers)
            ...
            def _weight_bar_cell(text_value, tone="tan"):
                try:
                    pct = float(str(text_value).replace("%", "").strip())
                except Exception:
                    return text_value
                ...
                return (
                    ...
                    f'<div style="width:78px;height:10px;border-radius:7px;background:{bg};overflow:hidden;border:1px solid #cfdae6;">'
...
# --------------------------------------------------
            with tab1:
                ...
                render_metric_card(c3, "Risk aversion score (gamma)", f"{gamma:.2f}")
                render_metric_card(c4, "ESG preference score (lambda)", f"{lambda_raw_avg:.2f} / 4")
...
# --------------------------------------------------
            with tab2:
                ...
                recommended_weights_view["Recommended Weight"] = recommended_weights_view["Recommended Weight"].map(
                    lambda x: _weight_bar_cell(x, "rec")
                )
                ...
                risky_compare_rows.extend([
                    {"Metric": "ESG score", "Recommended ESG Portfolio": f"{recommended['ESG_Score']:.2f}", "Tangency Portfolio": f"{tangency['ESG_Score']:.2f}"},
                    {"Metric": "ESG utility", "Recommended ESG Portfolio": f"{recommended['Utility']:.4f}", "Tangency Portfolio": "N/A"},
                    {"Metric": "Sharpe ratio", "Recommended ESG Portfolio": f"{recommended['Sharpe_Ratio']:.3f}", "Tangency Portfolio": f"{tangency['Sharpe_Ratio']:.3f}"},
                    {"Metric": "Expected return", "Recommended ESG Portfolio": f"{recommended['Expected_Return']*100:.2f}%", "Tangency Portfolio": f"{tangency['Expected_Return']*100:.2f}%"},
                    {"Metric": "Volatility", "Recommended ESG Portfolio": f"{recommended['Risk_SD']*100:.2f}%", "Tangency Portfolio": f"{tangency['Risk_SD']*100:.2f}%"},
                ])
                    ...
                    if not rec_uni.empty:
                        ...
                        if top3.empty:
                            ...
                            for _, row in top3.iterrows():
                                ...
                                badge_label, badge_color = alignment_badge(cs)
...
<div style="display:flex;justify-content:space-between;align-items:flex-start;flex-wrap:wrap;gap:0.5rem;">
  <div>
    <div style="font-size:1.08rem;font-weight:700;color:#e8edf8;">{row['ticker']}</div>
    <div style="font-size:0.83rem;color:#a8b8d8;margin-top:0.2rem;">{why}{vs_note}</div>
  </div>
  <div style="text-align:right;">
    <span style="display:inline-block;padding:0.2rem 0.6rem;border-radius:999px;background:{badge_color}22;color:{badge_color};font-size:0.8rem;font-weight:600;">{badge_label}</span>
    <div style="font-size:0.92rem;color:#34c759;font-weight:700;margin-top:0.2rem;">Alignment: {cs:.1f}/100</div>
  </div>
</div>
<div style="display:flex;gap:1.4rem;flex-wrap:wrap;margin-top:0.55rem;font-size:0.84rem;color:#a8f0bb;">
...
{gap_html}
                ...
                if st.button("Next ", key="next_tab4"):
                    st.session_state["active_tab"] = 4

# --------------------------------------------------
# TAB 5 - Your Compatibility Score
# --------------------------------------------------
            with tab5:
...
border:1px solid rgba(15,23,42,0.08);">
  ...
  <div style="font-size:0.95rem;color:#a8b8d8;margin-top:0.8rem;text-align:center;max-width:540px;">
    {verdict_text}
  </div>
</div>""",
                    unsafe_allow_html=True,
...
color:#a8b8d8;margin-bottom:0.3rem;">
  <span><strong>{label}</strong></span>
  <span style="color:{color};font-weight:700;">{pct}%</span>
</div>
<div style="background:rgba(13,28,64,0.85);border-radius:999px;height:10px;overflow:hidden;">

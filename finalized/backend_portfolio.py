...
def fetch_market_data(tickers, ticker2=None, period='3y'):
    ...
    def _download_close_series(raw_ticker):
        ...
        for candidate in candidates:
            try:
                data = yf.download(candidate, period=period, auto_adjust=True, progress=False, threads=False)
            except Exception:
                continue
...
def fetch_universe_returns(tickers, period):
    ...
    for i in range(0, len(mapped_download_list), chunk_size):
        ...
        try:
            data = yf.download(chunk, period=period, auto_adjust=True, progress=False, threads=False)
            if data.empty:
                continue
...
def _build_weight_samples(n_assets, sample_count=8000):
    if n_assets == 2:
...
def build_portfolio_table(expected_returns=None, covariance_matrix=None, rf=None, esg_scores=None, gamma=None, lambda_esg=None, tickers=None, sample_count=8000, **legacy_kwargs):
    ...
    weights = _build_weight_samples(len(mu), sample_count=sample_count)
    rows = []
    weight_cols = _weight_columns(tickers)
    for weight_vector in weights:
        ret = portfolio_return(weight_vector, mu)
        var = portfolio_variance(weight_vector, cov)
        risk = np.sqrt(var)
        esg_score = portfolio_esg(weight_vector, esg_vector)
        sharpe = (ret - rf) / risk if risk > 0 else np.nan
        utility = esg_utility_function(ret, var, esg_score, gamma, lambda_esg)
        row = {
            'Weights': {ticker: float(weight) for ticker, weight in zip(tickers, weight_vector)},
            'Expected_Return': ret,
            'Variance': var,
            'Risk_SD': risk,
            'ESG_Score': esg_score,
        ...
        for col_name, weight in zip(weight_cols, weight_vector):
            row[col_name] = float(weight)
        if len(weight_vector) >= 2:
            row['Weight_Asset1'] = float(weight_vector[0])
            row['Weight_Asset2'] = float(weight_vector[1])
        rows.append(row)
...
def add_end_label(ax, x_value, y_value, label, color, offset=(8, 0), boxed=True):
    kwargs = {}
    if boxed:
        kwargs['bbox'] = dict(boxstyle='round,pad=0.30', fc='#ffffff', ec='#d9e2ec', alpha=0.98)
    ax.annotate(
        label,
        (x_value, y_value),
...
def _efficient_frontier_curve(df):
    curve = df[['Risk_SD', 'Expected_Return']].dropna().sort_values(['Risk_SD', 'Expected_Return'])
    if curve.empty:
        return curve
...
def _esg_return_curve(df):
    curve = df[['ESG_Score', 'Expected_Return']].dropna().sort_values(['ESG_Score', 'Expected_Return'])
    if curve.empty:
        return curve
...
def _esg_sharpe_curve(df):
    curve = df[['ESG_Score', 'Sharpe_Ratio']].dropna().sort_values(['ESG_Score', 'Sharpe_Ratio'])
    if curve.empty:
        return curve
...
def make_frontier_figure(df, tangency, recommended, tickers, mean_returns, volatilities):
    ...
    recommended_color = '#3e8b66'
    tangency_color = '#2a3b57'
    ...
    add_end_label(ax, tangency['Risk_SD'], tangency['Expected_Return'], 'Tangency', tangency_color, offset=(8, -15))
    ax.annotate(
        f'Optimal ESG Portfolio\nExp. Return: {recommended["Expected_Return"]*100:.1f}%\nVolatility: {recommended["Risk_SD"]*100:.1f}%',
        ...
        fontsize=10,
        color='#1f2f46',
        bbox=dict(boxstyle='round,pad=0.55', fc='#fdfefe', ec='#d8e1eb', alpha=0.98),
    )
...
def make_cml_figure(df, tangency, complete_portfolio, rf, tickers, mean_returns, volatilities):
    ...
    add_end_label(ax, tangency['Risk_SD'], tangency['Expected_Return'], 'Tangency', tangency_color, offset=(8, -14))
    add_end_label(ax, complete_portfolio['Risk_SD'], complete_portfolio['Expected_Return'], 'Complete', complete_color, offset=(8, 8))
    ax.annotate(
        'Risk-free',
        (0, rf),
        textcoords='offset points',
        ...
        fontsize=9.8,
        color='#5b6578',
        bbox=dict(boxstyle='round,pad=0.25', fc='#ffffff', ec='#d9e2ec', alpha=0.98),
    )
    ax.set_xlabel('Portfolio Volatility')
    ax.set_ylabel('Expected Return')
...
def make_esg_tradeoff_figure(df, recommended):
    fig, ax = plt.subplots(figsize=(10, 6))
    fig.patch.set_facecolor('#f7f9fc')
    tradeoff_color = '#3e8b66'
    recommended_color = '#2f6f52'
    curve = _esg_return_curve(df)

    ax.plot(curve['ESG_Score'], curve['Expected_Return'], linewidth=2.8, color=tradeoff_color)
    ax.scatter(recommended['ESG_Score'], recommended['Expected_Return'], s=112, marker='o', color=recommended_color, edgecolors='white', linewidths=1.0, zorder=4)
    if not curve.empty:
        add_end_label(ax, curve['ESG_Score'].iloc[-1], curve['Expected_Return'].iloc[-1], 'Trade-off', tradeoff_color, offset=(9, -9))
    add_end_label(ax, recommended['ESG_Score'], recommended['Expected_Return'], 'Recommended', recommended_color, offset=(8, -14), boxed=True)
    ax.set_xlabel('Portfolio ESG Score')
    ax.set_ylabel('Expected Return')
    _chart_title(ax, 'ESG Frontier & Recommended Portfolio')
...
def make_esg_efficient_frontier_figure(df, tangency, recommended):
    ...
    ax.scatter(recommended['ESG_Score'], recommended['Sharpe_Ratio'], s=110, marker='o', color=recommended_color, edgecolors='white', linewidths=1.1, zorder=4)
    x_left = min(tangency['ESG_Score'], recommended['ESG_Score'])
    x_right = max(tangency['ESG_Score'], recommended['ESG_Score'])
    y_bottom = min(tangency['Sharpe_Ratio'], recommended['Sharpe_Ratio'])
    y_top = max(tangency['Sharpe_Ratio'], recommended['Sharpe_Ratio'])
    ax.hlines(
    ...
    add_end_label(ax, tangency['ESG_Score'], tangency['Sharpe_Ratio'], 'Tangency', tangency_color, offset=(8, -14))
    add_end_label(ax, recommended['ESG_Score'], recommended['Sharpe_Ratio'], 'ESG optimal', recommended_color, offset=(8, -14))
    ...
    ax.annotate(
        ...
        fontsize=9.6,
        color='#49606e',
        bbox=dict(boxstyle='round,pad=0.45', fc='#f8fbf9', ec='#c9ddd5', alpha=0.98),
    )
    ax.set_xlabel('Portfolio ESG Score')
...
def make_price_history_figure(prices, tickers):
    ...
    for idx, ticker in enumerate(tickers):
        color = palette[idx]
        ax.plot(prices.index, prices[ticker], linewidth=2.6 if idx < 2 else 2.0, color=color, label=ticker)
        add_end_label(ax, prices.index[-1], prices[ticker].iloc[-1], ticker, color, offset=(8, -2 - (idx * 10)))
    _chart_title(ax, 'Historical Price Performance')
    ax.set_xlabel('Date')
